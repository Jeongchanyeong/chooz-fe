...
`useInfiniteQuery`를 사용했지만 일관된 UI 트리거 패턴으로 감싸지 못하고 있다는 생각이 들었습니다.
...
- 피드, 알림, 내 투표 목록 등 여러 리스트 화면에 동일한 무한 스크롤 패턴을 재사용
- 화면마다 작성하던 scroll 위치 계산 및 중복 요청 방지 조건문을 제거
- 추가 요청 조건이 한 곳으로 모여 중복 호출 방지 기준을 일관되게 유지
- 신규 리스트 화면 추가 시 감지 로직을 다시 구현하지 않고 기존 패턴을 그대로 적용
