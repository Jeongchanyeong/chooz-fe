...
- 사용자 생성 콘텐츠(UGC) 서비스의 사후 대응 중심 운영 구조
...
`REST API → React Query → Custom Hooks → Zustand / UI`
...
같은 엔터티를 서로 다른 key로 관리하면서 캐시 구조가 일관되지 않았고, mutation 이후 어떤 범위를 갱신해야 하는지 추론하기 어려웠습니다.
...
피드, 알림, 내 투표 목록 등 여러 리스트 화면에서 무한 스크롤이 필요했지만 화면마다 구현 방식이 달라 동일한 로직을 반복 작성하고 있었습니다.

아래와 같은 문제가 발생하였습니다.

- scroll 이벤트를 직접 연결해 화면마다 별도 조건문 작성
- 로딩 중에도 추가 요청이 들어가 중복 호출 발생
- 감지 로직과 페이징 로직이 한 파일 안에 섞여 재사용이 어려운 구조

리스트 화면이 늘어날수록 중복 구현 비용이 커지는 상태였습니다.


---
### Cause

스크롤 감지와 데이터 요청이 하나의 흐름에 결합되어 있었습니다.

```
scroll event -> 현재 스크롤 위치 계산 -> 조건 검사 -> fetchNextPage()
```

화면마다 위치 계산, 가드 조건, 중복 요청 방지 로직을 직접 처리해야 했고, 감지 책임과 요청 책임이 분리되지 않았습니다.

---
### ✅ Solution

무한 스크롤을 감지 계층과 요청 계층으로 분리했습니다.
...
브라우저의 `IntersectionObserver`를 사용해 스크롤 위치를 직접 계산하지 않고 마지막 요소가 viewport 근처에 들어왔는지만 감지하도록 변경했습니다.
...
- isFetchingNextPage → 중복 요청 방지

---
### Result

- 화면별 중복 구현 없이 동일한 무한 스크롤 패턴을 재사용할 수 있게 됨
- 중복 요청 방지 로직이 공통화되어 안정성이 높아짐
- 감지 책임과 데이터 요청 책임이 분리되어 유지보수성이 향상됨
...
유해 데이터가 시스템 내부에 저장된 뒤 대응하는 구조였고, 운영 비용과 대응 속도가 모두 인력에 의존했습니다.
...
구조를 분리한 이유는 다음과 같습니다.

- OpenAI API Key를 프론트엔드에 두지 않기 위해
- 판별 로직과 UI 로직을 분리하기 위해
- 정책 로직을 한 곳에서 관리하고 재사용하기 위해
...
- 유해 표현을 저장 이후가 아니라 작성 시점에 선제적으로 차단할 수 있게 됨
- 운영자의 사후 검수 부담을 줄일 수 있는 기반을 마련함
- 프론트·서버·정책 로직의 책임 분리가 가능해짐
- 정책 변경이나 모델 교체에 대응하기 쉬운 구조를 확보함
...
- API 준비 전까지 화면 구현 지연
- 임시 더미 데이터가 컴포넌트 내부에 흩어짐
- 실제 네트워크 흐름과 다른 방식으로 개
...
React Query를 도입했다고 캐시 문제가 자동으로 해결되지는 않습니다.ㅌ
queryKey를 어떻게 설계하느냐가 곧 데이터 흐름의 품질과 유지보수성을 결정함을 깨달았습니다.

2. UGC 서비스의 품질은 기능보다 운영 구조에서 갈린다

게시글 작성 기능보다 중요한 것은 유해 콘텐츠를 언제, 어디서, 어떤 비용으로 통제할 것인지입니다.
Moderation은 기능 추가가 아니라 운영 구조 설계에 가까운 문제였습니다.

3. 좋은 자동화는 사람을 대체하는 것이 아니라 판단 비용을 줄인다

AI 리뷰, CI 캐시, MSW는 모두 반복 작업을 줄이고 사람이 더 중요한 문제를 보게 만들기 위한 장치였습니다.
